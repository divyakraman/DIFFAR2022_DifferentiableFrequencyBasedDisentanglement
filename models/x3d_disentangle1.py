...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1_s(x) #24,8,480,270 for input 8, 960, 540
        x = self.conv1_t(x) #24,8,480,270 for input 8, 960, 540
        x = self.bn1(x)
        x = self.relu(x)

        x = self.layer1(x) #24,8,240,135 for input 8, 960, 540
        
        x = self.layer2(x) #48,8,120,68 for input 8, 960, 540
        
        mask1 = self.objback_disentangle(x)
      
        x = self.layer3(x) #96,8,60,34 for input 8, 960, 540
