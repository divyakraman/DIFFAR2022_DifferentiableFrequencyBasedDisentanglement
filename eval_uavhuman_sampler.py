...
from dataset.uav_human_all import UAVHumanDataSet
...
for i in range(0, NUM_STEPS_STOP):
	...
	images = frameSampler1(all_frames, uniform_frames, NUM_FRAMES, model_link)
	images = images.permute(0,2,1,3,4) #For I3D
	images = Variable(images).cuda()
	out_uniform = net(uniform_frames.permute(0,2,1,3,4))
	out_uniform = out_uniform[0]
	#out_uniform = out_uniform[:,:,0] #Only for X3D
	out_uniform = out_uniform.detach()
	out = net(images)
	out = out[0]
