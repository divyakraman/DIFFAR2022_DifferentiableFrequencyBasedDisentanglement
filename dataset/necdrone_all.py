...
class NECDataSet(data.Dataset):
	...
	def __getitem__(self, index):
		...
		x_all = torch.zeros(num_frames_video, 3, int(1920/self.input_size_scale), int(1080/self.input_size_scale))
		...
		for i in range(0, num_frames_video):
			frame = cv2.imread(video_images_list[i])
			frame = cv2.resize(frame,(int(1080/self.input_size_scale), int(1920/self.input_size_scale)))
			frame = np.asarray(frame)
			frame = torch.from_numpy(frame).permute(2,0,1)/255.0
			x_all[i,:,:,:] = frame
		
		return x, gt_label, x_all
