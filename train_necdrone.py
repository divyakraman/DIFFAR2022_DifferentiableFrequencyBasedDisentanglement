...
from dataset.necdrone import NECDataSet
...
DATA_DIRECTORY = ''
DATA_LIST_PATH = './dataset/NECDrone/trainval.txt'
INPUT_SIZE_SCALE = 2
LEARNING_RATE = 0.01
MOMENTUM = 0.9
...
RANDOM_SEED = 1234
SAVE_PRED_EVERY = 50
SNAPSHOT_DIR = './snapshots/nec_x3d_disentangle1/'
WEIGHT_DECAY = 0.0005
...
def main():
	...
	#pretrained = torch.load('', map_location='cpu').state_dict()
	
	net = generate_model(x3d_version='M', n_classes=400, n_input_channels=3, dropout=0.5, base_bn_splits=1)
	...
	trainloader = data.DataLoader(
		NECDataSet(DATA_DIRECTORY, DATA_LIST_PATH, max_iters=NUM_STEPS_STOP * ITER_SIZE * BATCH_SIZE,
	...
	for i_iter in range(0, NUM_STEPS_STOP):
		...
		if i_iter >= NUM_STEPS_STOP - 1:
			print('save model ...')
			torch.save(net, osp.join(SNAPSHOT_DIR, 'NEC_' + str(NUM_STEPS_STOP) + '.pth'))
			break
